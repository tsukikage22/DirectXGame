...
public:
    ...
    bool LoadHDRI(const std::filesystem::path& filePath, DirectX::ResourceUploadBatch& batch);
    ...
    static constexpr uint32_t kPrefilteredSize        = 128;                            // prefiltered mapのサイズ
    static constexpr uint32_t kPrefilteredMipLevels   = 5;                              // prefiltered mapのミップ
    static constexpr uint32_t kBrdfLutSize            = 512;                            // BRDF LUTのサイズ
    static constexpr DXGI_FORMAT kPrefilteredFormat   = DXGI_FORMAT_R16G16B16A16_FLOAT; // prefiltered mapのフォーマット
    static constexpr DXGI_FORMAT kCubemapFormat       = DXGI_FORMAT_R16G16B16A16_FLOAT; // キューブマップのフォーマット
    static constexpr DXGI_FORMAT kIrradianceMapFormat = DXGI_FORMAT_R32G32B32A32_FLOAT; // 照度マップのフォーマット
    static constexpr DXGI_FORMAT kBrdfLutFormat       = DXGI_FORMAT_R16G16_FLOAT;       // BRDF LUTのフォーマット
...
private:
    ...
    TextureResource m_defaultCubeMap; // デフォルトキューブマップテクスチャのリソース
