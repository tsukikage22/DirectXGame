...
struct PhotometricLightDesc
{
    ...
    float luminousFlux               = 10000.0f;              // 光束[lm]
    float range                      = 30.0f;                 // 影響範囲
    std::optional<uint32_t> iesIndex = std::nullopt;          // IESプロファイルのインデックス
};
...
public:
    ...
    {
        return m_innerAngleDeg;
    }
    float GetOuterAngle() const
    {
        return m_outerAngleDeg;
    }

    //============================================
    // Photometric light parameter
    //============================================
    void SetIESIndex(std::optional<uint32_t> index)
    {
        m_iesIndex = index;
    }
...
private:
    ...
    DirectX::XMFLOAT3 m_color = { 1.0f, 1.0f, 1.0f }; // 相対輝度1に正規化した色度
