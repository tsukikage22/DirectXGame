...
static_assert(std::size(kDebugViewNames) == static_cast<size_t>(DebugView::size),
...
struct SceneConstants
{
    ...
    DirectX::XMFLOAT4X4 lightViewProj; // ライトのビュー射影行列（シャドウマッピング用）
...
struct LightConstants
{
    ...
    uint32_t type;              // ライトの種類（0: ディレクショナルライト, 1: ポイントライト, 2:
...
static_assert(sizeof(LightConstants) == 64, "Must be matched with shader struct size");
