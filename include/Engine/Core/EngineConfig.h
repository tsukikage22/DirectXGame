...
namespace config
{
inline constexpr DXGI_FORMAT kBackBufferFormat  = DXGI_FORMAT_R16G16B16A16_FLOAT; // バックバッファのフォーマット
inline constexpr DXGI_FORMAT kDepthBufferFormat = DXGI_FORMAT_D32_FLOAT;          // 深度バッファのフォーマット
inline constexpr DXGI_FORMAT kShadowMapFormat   = DXGI_FORMAT_D32_FLOAT;          // シャドウマップの深度フォーマット
inline constexpr DXGI_FORMAT kUIBufferFormat    = DXGI_FORMAT_R8G8B8A8_UNORM;     // UI用バッファのフォーマット
...
// 3x3PCFを使用する場合は、4x4のテクセルを参照するため、4.0fに設定する
inline constexpr float kShadowPcfFootprintTexels = 4.0f;
...
inline constexpr float kShadowSlopeScaledBias = kShadowPcfFootprintTexels * kShadowDiagonalCompensation;
// 深度バイアス
inline constexpr int kShadowDepthBias = 10000;
// 深度バイアスクランプ
inline constexpr float kShadowDepthBiasClamp = 0.01f;
...
// CBV/SRV/UAVヒープの最大数
inline constexpr uint32_t kCbvSrvUavCapacity = kMaxObjects * kFrameCount // Transform CBV
                                               +
...
inline constexpr uint32_t kRtvCapacity      = kFrameCount + 8 + 32; // バックバッファ，キューブマップ，UI描画RT
