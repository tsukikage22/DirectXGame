...
// コンストラクタ
CommandQueue::CommandQueue() : m_pFence(nullptr), m_nextFence(1), m_fenceEvent(nullptr)
{
}
...
bool CommandQueue::Init(ID3D12Device* pDevice, D3D12_COMMAND_LIST_TYPE type)
{
    ...
    CHECK_HR(pDevice, pDevice->CreateCommandQueue(&desc, IID_PPV_ARGS(m_pQueue.GetAddressOf())));

    CHECK_HR(pDevice, pDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_pFence.GetAddressOf())));
...
ID3D12CommandQueue* CommandQueue::GetD3DQueue() const
{
    return m_pQueue.Get();
}
