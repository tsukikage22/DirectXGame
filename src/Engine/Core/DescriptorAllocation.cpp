...
// コンストラクタ
DescriptorAllocation::DescriptorAllocation(DescriptorPool* pPool, uint32_t index) : m_pPool(pPool), m_index(index)
{
}

// コンストラクタ（連続領域確保用）
DescriptorAllocation::DescriptorAllocation(DescriptorPool* pPool, uint32_t index, uint32_t count)
...
/// @brief CPUハンドルの取得
D3D12_CPU_DESCRIPTOR_HANDLE DescriptorAllocation::GetCPUHandle(uint32_t offset) const
{
    if (!IsValid())
    {
...
/// @brief GPUハンドルの取得
D3D12_GPU_DESCRIPTOR_HANDLE DescriptorAllocation::GetGPUHandle(uint32_t offset) const
{
    if (!IsValid())
    {
        assert(false && "Invalid DescriptorAllocation");
