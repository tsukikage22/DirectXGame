...
AssetLoadScope::AssetLoadScope(std::unique_ptr<DirectX::ResourceUploadBatch> pbatch, CommandQueue& queue,
    ModelLoader& loader, Scene& scene, IESProfile& iesProfile)
    : m_pbatch(std::move(pbatch)), m_queue(queue), m_loader(loader), m_scene(scene), m_iesProfile(iesProfile) {};
...
engine::ModelHandle AssetLoadScope::LoadModel(const std::filesystem::path& path)
{
...
std::optional<uint32_t> AssetLoadScope::LoadIESProfile(const std::filesystem::path& path)
{
