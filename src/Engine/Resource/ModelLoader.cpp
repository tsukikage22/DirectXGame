...
bool ModelLoader::Init(GraphicsDevice& graphicsDevice, TextureManager& textureManager)
{
...
std::unique_ptr<Model> ModelLoader::LoadModel(const std::filesystem::path& path, DirectX::ResourceUploadBatch& batch)
{
    // 初期化チェック
    ...
    if (!model->Init(*m_pGraphicsDevice, m_pTextureManager, batch, modelAsset))
    {
        return nullptr;
    }
