...
MaterialGPU::MaterialGPU()
    : m_constantBuffer(), m_pTextureManager(nullptr), m_constants(), m_baseColorIndex(std::nullopt),
      m_metallicRoughnessIndex(std::nullopt), m_normalIndex(std::nullopt), m_emissiveIndex(std::nullopt),
...
bool MaterialGPU::Init(
    GraphicsDevice& graphicsDevice, TextureManager* pTextureManager, const MaterialAsset& materialAsset)
{
    // 引数チェック
    if (!pTextureManager)
    {
    ...
    // 定数バッファの作成
    if (!m_constantBuffer.Init<shader::MaterialConstants>(graphicsDevice.GetDevice(), graphicsDevice.CbvSrvUavPool()))
    {
        return false;
    }
    ...
    // SRVテーブルの初期化
    if (!m_srvTable.Init(graphicsDevice.GetDevice(), graphicsDevice.CbvSrvUavPool(), this, pTextureManager))
    {
        return false;
    }
...
std::optional<uint32_t> MaterialGPU::GetTextureHandle(TextureUsage usage) const
{
    switch (usage)
    {
    case TextureUsage::BaseColor:
