...
bool LightBuffer::Init(ID3D12Device* pDevice, DescriptorPool* pPoolSRV)
{
    ...
    // バッファの作成
    if (!m_buffer.CreateDynamic(pDevice, sizeof(shader::LightConstants) * config::kMaxLights))
    {
        return false;
    }
    ...
    pDevice->CreateShaderResourceView(m_buffer.GetResource(), &srv, m_allocation.GetCPUHandle());
...
uint32_t LightBuffer::Update(const shader::LightConstants* pLights, uint32_t count)
{
    // 引数チェック
