...
DisplayConstantsGPU::~DisplayConstantsGPU()
{
    Term();
}

bool DisplayConstantsGPU::Init(
    ID3D12Device* pDevice, DescriptorPool* pPoolCBV, const shader::DisplayConstants& displayConstants)
{
    // 引数チェック
    if (!pDevice || !pPoolCBV)
    {
...
void DisplayConstantsGPU::Term()
{
    m_constantBuffer.Term();
}

void DisplayConstantsGPU::Update(const shader::DisplayConstants& displayConstants)
{
